        ...
        <div>
          <ul className="mt-12 gap-y-6 gap-x-12 items-center md:flex lg:gap-x-0 lg:mt-0">
            {contactMethods.map((item, idx) => (
              ...
              >
                <div className="w-12 h-12 rounded-full border flex items-center justify-center opacity-80">
                  {item.icon}
                </div>
                <h4 className="opacity-90 text-lg font-medium xl:text-xl">
                  {item.title}
                </h4>
                <p>{item.desc}</p>
                <Link
                  href={item.link.href}
                  className="link link-hover link-primary flex items-center"
                >
                  {item.link.name}
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
